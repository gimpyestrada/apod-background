...
def extract_image_url(apod: Optional[dict]) -> tuple[Optional[str], bool]:
    ...
    could be found; is_video is True when the entry is a video, not an image.
    """
    if not apod:
        return None, False
...
def main() -> int:
    ...
    image_url, is_video = extract_image_url(apod)
    if not image_url:
        if not is_video:
